...
import { decryptPat, destroyBundle } from "@/lib/databricks";
...
/**
 * POST /api/databricks/destroy
 * Destroys a Databricks Asset Bundle for a mission.
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = destroyRequestSchema.safeParse(body);
    ...
    // Destroy the bundle
    const result = await destroyBundle(
      userId,
      missionSlug,
    ...
    if (result.status === "error") {
      return NextResponse.json(
        { error: result.error || "Failed to destroy bundle" },
        { status: 500 }
      );
    }
    ...
    console.error("[api/databricks/destroy]", error instanceof Error ? error.message : "Unknown error");
    return NextResponse.json(
