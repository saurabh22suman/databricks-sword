...
- **Unity Catalog Volumes** (recommended) or cloud storage (S3/ADLS/GCS) for storing data
...
- No Unity Catalog
- No SQL Warehouses
- Cluster auto-terminates after an inactivity window configured by the platform
...
- **Data** — browse tables and volumes
...
> **Note:** Community Edition clusters can auto-terminate after inactivity. Check your current compute settings in the Databricks UI and restart from the Compute page when needed.
...
## Step 6: Run Missions Against Your Workspace

Databricks Sword integrates with your Databricks workspace through the **Profile → Settings → Databricks Connection** panel. Once connected:

1. Your workspace URL and PAT are stored securely.
2. Select missions that support real execution (no Unity Catalog or Workflows).
3. Code challenges execute against your real Spark cluster instead of simulation.

> **Note:** Not all missions support real Databricks execution. Stage types like drag-drop and quiz are always simulated. Code challenges (fill-blank, free-text) can execute against your workspace when connected.
...
Your cluster may auto-terminate after inactivity:
