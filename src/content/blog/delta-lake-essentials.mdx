...
Use `CLUSTER BY` for adaptive data layout. It replaces both partitioning and Z-ORDER for new tables:

```sql
CREATE TABLE sales ( ... ) CLUSTER BY (sale_date, product_id);

-- Change clustering columns without rewriting data
ALTER TABLE sales CLUSTER BY (product_id, sale_date);

-- Apply clustering incrementally
OPTIMIZE sales;
```

> **Legacy note:** `ZORDER BY` is still available but requires a full rewrite to change columns. `partitionBy` is supported for backward compatibility but creates cardinality-related maintenance issues; prefer Liquid Clustering for new tables.
...
### 1. Use Liquid Clustering (Databricks)
