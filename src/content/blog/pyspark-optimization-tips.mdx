...
df_processed.persist(StorageLevel.MEMORY_AND_DISK)  # explicit level preferred over .cache()
...
AQE is **enabled by default** in Databricks Runtime and Spark 3.2+. It re-optimizes query plans at runtime based on actual data statistics, automatically handling:

- **Skew joins:** splits skewed partitions automatically
- **Partition coalescing:** merges small partitions after shuffle
- **Join strategy switching:** converts sort-merge joins to broadcast joins at runtime when stats show a small side

You usually don't need to configure it, but if disabled:
```python
spark.conf.set("spark.sql.adaptive.enabled", "true")
```
