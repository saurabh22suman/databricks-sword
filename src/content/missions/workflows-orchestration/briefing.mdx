...
3. **Configure Serverless Jobs** — instant startup, zero-config provisioning, automatic scale-to-zero
...
- Tasks can be notebooks, JARs, Python scripts, SQL queries, or Spark Declarative Pipelines (formerly Delta Live Tables)
- Serverless Jobs are the recommended default for most workloads — instant startup, automatic scaling, no cluster configuration. Classic Job Clusters remain available as a legacy fallback for specialized use cases.
