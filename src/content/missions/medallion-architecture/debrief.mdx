...
- **Partitioning / Liquid Clustering** — prefer `CLUSTER BY` for adaptive layout; `partitionBy` is legacy for new tables
...
- **Quality rules** — SDP expectations (`dp.expect`, `dp.expect_or_drop`, `dp.expect_or_fail`) filter invalid records
...
- **Optimized for consumers** — clustered via `CLUSTER BY` for query patterns, or rely on Predictive Optimization for automatic maintenance

### Predictive Optimization
- Enable Predictive Optimization on Gold tables to let the platform schedule OPTIMIZE and VACUUM automatically
- The platform learns table usage patterns and runs maintenance without manual intervention
- Significantly reduces operational overhead for high-cardinality, continuously growing tables

### CDC Through Medallion
- **APPLY CHANGES INTO** — SDP's CDC operator (`dp.apply_changes`) for propagating updates
...
- Separate SDP pipelines per domain for independent scaling
...
4. Use SDP for automatic dependency management between layers
5. Monitor pipeline health with the SDP event log and quality metrics
