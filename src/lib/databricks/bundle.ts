...
export async function generateBundle(
  ...
  const databricksYml = generateDatabricksYml(industry, config.workspaceUrl, schemaPrefix)
 ...
 * Bundle name and workspace root_path are hardcoded per deployment to keep
 * DAB's identity stable and to land files at the path the UI links to.
 */
function buildDatabricksYmlObject(
  industry: Industry,
  workspaceUrl: string,
  schemaPrefix: string
  ...
  const yml: Record<string, unknown> = {
    bundle: {
      // Unique per deployment so DAB treats each as a separate bundle
      // (root_path differs per deployment, so name must too).
      name: `field-ops-${industry}-${schemaPrefix}`,
      // bundle.uuid is auto-generated by DAB. Do NOT set it to "${bundle.uuid}"
      // — that creates a self-reference cycle:
      //   Error: cycle detected in field resolution: bundle.uuid -> bundle.uuid
    },
    workspace: {
...
function generateDatabricksYml(
  ...
  const obj = buildDatabricksYmlObject(industry, workspaceUrl, schemaPrefix)
