...
export const INDUSTRY_CONFIGS: Record<Industry, IndustryConfig> = {
  ...
  manufacturing: {
    ...
    description: "Quality prediction with Spark Declarative Pipelines and Feature Store",
    scenario: "PRODUCTION HALT: PrecisionTech's semiconductor fab is producing 23% defective chips—triple the acceptable rate. The $2B factory is bleeding money every hour it runs. Quality engineers are baffled; the defects seem random but the pattern must be hidden in the sensor data. 847 parameters per wafer, 10,000 wafers per day, and somewhere in that tsunami of data is the answer. Your mission: build a quality prediction system using Spark Declarative Pipelines (formerly DLT) and Feature Store to catch defects before they happen.",
    objectives: [
      "Ingest manufacturing sensor data with SDP to Bronze",
  ...
  "medtech-research": {
    ...
    description: "RAG over medical research papers with Vector Search, Foundation Model, and a Streamlit app",
    scenario: "BREAKTHROUGH NEEDED at HelixLabs: A team of oncology researchers is drowning in 10,000+ new medical papers per month. They need a private, trustworthy Q&A system that can answer questions like 'What Phase II trials in 2024 combined pembrolizumab with lenvatinib for HCC?' using their own curated corpus — not the open internet. Foundation models hallucinate. Keyword search misses synonyms. The RAG pattern is the answer, and you'll build it: a Vector Search index synced from a Delta table, an embeddings pipeline, a foundation model serving endpoint, and a Streamlit app in Databricks Apps.",
    objectives: [
      "Deploy Vector Search endpoint and Delta Sync index over the medical papers Delta table",
      "Build embeddings pipeline using ai_embed() to populate the index",
      "Deploy a foundation model serving endpoint for the LLM",
      "Deploy a Streamlit RAG app that retrieves and grounds answers",
      "Pass all 5 validation queries with 100% accuracy",
    ],
    xpRequired: 16000,
    xpReward: 1200,
